...
class ImgPicker
{
	 ...
	 *  @return void
	 */
	function __construct($options = array(), $error_messages = array(), $type=null)
	{
                if(!empty($type) && $type == "testimonial"){
                        $min_width  = 1100;
                        $min_height = 825;
		...
		$this->options = array(
			...
			// Accepted file types:
			'accept_file_types' => 'png|jpg|jpeg|gif|ico',
	...
	protected function load()
	{
		...
		foreach ($files as $file) {
			...
			foreach ($this->options['versions'] as $version => $options) {
				...
				$filename = $this->getVersionFilename($image->name, $version, $name);
	...
	protected function delete()
	{
		...
		if (file_exists($filepath) && call_user_func($this->options['delete'], $filename, $this)) {
			foreach ($this->options['versions'] as $version => $options) {
				$name = isset($options['name']) ? $options['name'] : '';
				$name = $this->getVersionFilename($filename, $version, $name);
	...
	protected function crop()
	{
		...
		if (in_array(abs($rotate), array(90, 180, 270))) {
			$angle = ($rotate < 0) ? abs($rotate) : 360 - $rotate;
			$this->rotateImage($image->path, $angle);
		}

		$this->resizeImage($image->path, null, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);

		list($image->width, $image->height) = @getimagesize($image->path);

		$image->versions = $this->generateVersions($image);
		...
		if ($image->path != $this->getUploadPath($image->name)) {
			foreach ($this->options['versions'] as $version => $options) {
				$name = isset($options['name']) ? $options['name'] : '';
				$filename = $this->getVersionFilename(basename($image->path), $version, $name);
	 ...
	 *  @return string
	 */
	public function getVersionFilename($filename, $version, $name = '')
	{
		...
		if ($name != '') {
			...
			return sprintf($name, $filenameWithoutExtension).'.'.$ext;
		}
	...
	{
		return pathinfo(strtolower($filename), PATHINFO_FILENAME);
	}
